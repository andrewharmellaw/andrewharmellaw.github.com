...
The next section of [Gelfand](https://www.amazon.co.uk/Algebra-I-M-Gelfand/dp/0817636773) (number 38) launches straight in and it can take a while to figure out where things are going and what the point is.  I've discovered as I've been trying to make full and proper sense of all this (to assist chunking as much as anything else) that Gelfand is assuming you've got a dictionary to hand; sometimes the signal of where you're going is as slight as a word in a section title.  The key one here? "Interpolation".

So what is interpolation?  I'd suggest you take a break to glance at [the wikipedia page](https://en.wikipedia.org/wiki/Interpolation).  Just glance mind, as the stuff that's about to come from Gelfand will give you the tools to grokk the contents within.

For those who don't want to take a look, Interpolation is defined on the page above as "_a method of constructing new data points within the range of a discrete set of known data points_"
...
Once we've done this (and had another encounter with Euler) Gelfand poses the question upon which he spends the rest of the section, and I'll spend the rest pf this chunk exploring.  "What can be said about a polynomial if we have some information about its values?"
...
Lets step up our game one more notch.  What about a second-degree polynomial?  You guessed it, it _must_ have an element in the form $ax^2$ and may have elements in the form $bx^1$ and $cx^0$.

We can keep this pattern going ad-infinitum.  But rather than that, lets bring it back to Gelfand so we're all happy we linked back up after diverging explanations.  We could write our second-degree with less words as follows:
...
Which is where Gelfand is waiting for us.

## Deplying Our New-Found Powers (and Point Two)
I mentioned a few paragraphs above that there were _two_ points.  We're about to see the second.  Gelfand now gives us some problems, assumes we're going to use the pattern we just learned, and some other basic skills he's taught us, and is hoping we easily get to the answer.
...
Bringing his worked example to a close, Gelfand points out what he's hoped we'd notice - you can use this trick for any polynomial of degree not exceeding one given two values of $x$.
...
First up, we need to grokk what he's saying - when he says "uniquely defined by three of its values" he means that if two polynomials are defined by the same three values, then they are the same.  He then takes this, and writes it with symbols rather than words
...
With all this in mind Gelfand then breaks out another proof-trick we've not seen before (at least I don't think so).  He introduces a third polynomial $R(x)$.  Why?  Lets rewind one step.

Gelfand is taking advantage of a property of polynomial roots which I'd forgotten about - that is that a polynomial of a dergee not exceeding 2 can have at most two roots, _unless it is equal to zero_.  It's the bit in _italics_ that is being used here.
...
because extrapolting what we had previously
...
Which means that $P(x) = Q(x)$ so they are the same, so therefore "a polynomial of degree not exceeding $2$ is defined uniquely by three of its values."
